(() => {
  ...
  function fakePost(company) {
    ...
    const html = `
      <h1 class="entry-title">${escapeHtml(company.name || company.mst)}</h1>
      <table class="taxinfo">
        <tr><th colspan="2">${escapeHtml(company.name || company.mst)}</th></tr>
        ${row('Tên quốc tế', company.internationalName)}
        ${row('Tên viết tắt', company.shortName)}
        ${row('Mã số thuế', company.mst)}
        ${row('Địa chỉ', company.address)}
        ${row('Người đại diện', company.representative)}
        ${row('Ngày hoạt động', company.startDate)}
        ${row('Loại hình DN', company.businessType)}
        ${row('Tình trạng', company.status)}
        ${row('Ngành nghề chính', company.industryName)}
      </table>
      ${updated}
      ${industryTable}`;

    return { content: { rendered: html } };
  }
  ...
  async function adaptSearch(keyword, init) {
    ...
    if (/^\d{10}(?:-?\d{3})?$/.test(q)) {
      ...
      return [{
        id: company.mst,
        title: `${company.mst} - ${company.name || 'Doanh nghiệp'}`,
  ...
  window.fetch = async (input, init) => {
    ...
    if (url.hostname === 'topmst.com' && url.pathname === '/api/v1/search') {
      try {
        const data = await adaptSearch(url.searchParams.get('s') || '', init);
        return jsonResponse(data);
      } catch (e) {
        return jsonResponse({ error: e.message || 'Không thể tra cứu nguồn dữ liệu.' }, e.status || 502);
      }
    }
    ...
    if (url.hostname === 'topmst.com' && url.pathname.startsWith('/api/wp/v2/posts/')) {
      ...
      try {
        const company = await getCompany(mst, init);
        return jsonResponse(fakePost(company));
      } catch (e) {
        return jsonResponse({ error: e.message || 'Không thể lấy chi tiết doanh nghiệp.' }, e.status || 502);
      }
    }
